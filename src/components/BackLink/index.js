...
const transition = createTransition(TransitionType.SLIDE, {appear: true, back: true});

export default function BackLink({ path, text, marginBottom }) {
  return (
    <Link to={path} transition={transition} className={cn('backLink', marginBottom && 'marginBottom' )}>
